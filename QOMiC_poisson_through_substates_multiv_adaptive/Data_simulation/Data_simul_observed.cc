...
//        }
            ...
            current_mu=(double)(atof(argv[++na]));
            if (na+1==argc) break;
            na++;
        }
        else if ( 0 == strcmp(argv[na],"-lambda1") ){
            current_lambda1=(double)(atof(argv[++na]));
            if (na+1==argc) break;
            na++;
        }
        else if ( 0 == strcmp(argv[na],"-lambda2") ){
            current_lambda2=(double)(atof(argv[++na]));
            if (na+1==argc) break;
            na++;
        }
        else if ( 0 == strcmp(argv[na],"-lambda3") ){
            current_lambda3=(double)(atof(argv[++na]));
            if (na+1==argc) break;
            na++;
        }
        else if ( 0 == strcmp(argv[na],"-lambda4") ){
            current_lambda4=(double)(atof(argv[++na]));
            if (na+1==argc) break;
            na++;
        }
        else if ( 0 == strcmp(argv[na],"-gamma") ){
            current_gamma=(double)(atof(argv[++na]));
            if (na+1==argc) break;
            na++;
        }
        else{
...
//    }
    ...
    string OutputName_PSI=Get_scen_name("./Results/Trajectories",
                                        filename_scenario,
                                        Extension);
...
//    mat_MCMC.Read_from_file(filename_in_MCMC);
    ...
    //    // Computing initial and end dates (i.e. smallest DOB and biggest date of diagnosis)
    //
    //    unsigned int init_date=30000000;
    //    for(unsigned int ind=0;ind<n_ind;ind++){
    //        init_date=std::min((double)(init_date), (double)(mat_dates.matrix[ind*mat_dates.nb_columns+0]));
    //    }
    //    init_date/=10000;
    //    cout << init_date << endl;
    //
    ...
    //    for(unsigned int ind=0;ind<n_ind;ind++){
    //        end_date=std::max((double)(end_date), (double)(mat_dates.matrix[ind*mat_dates.nb_columns+2]));
    //    }
    //    end_date/=10000;
    //    cout << end_date << endl;
    //
    ...
    //    // Computing end year
    //
    //    unsigned int end_year=end_date-init_date;
    //    cout << end_year << endl;
    //    cout << n_years_follow_up << endl;
    //
    //    mat_dates.Free_double_matrix_cont();
    //    gsl_matrix_free(mat_dates_work);
    
    
    // Initialisation of the matrix of trajectories
    
    Int_Matrices_cont mat_trajectories;
    mat_trajectories.Alloc_int_matrix_cont(n_ind,
                                           n_years_follow_up+1);
    //    mat_trajectories.Display_matrix_header();
...
//    }
    ...
    for(unsigned int ind=0;ind<n_ind;ind++){
        ...
        while(Stop_simul==0 && year<(n_years_follow_up)){
            ...
            double current_P_SI_R=mat_P_SI_R.matrix[ind*mat_P_SI_M.nb_columns+year];
            double current_P_SI_SI=mat_P_SI_SI.matrix[ind*mat_P_SI_M.nb_columns+year];
            double current_P_SI_M=mat_P_SI_SI.matrix[ind*mat_P_SI_M.nb_columns+year];
            ...
            if(DEBUG==0){
                ...
                cout << "Proability of transition SI-SI: " << current_P_SI_SI << endl;
                cout << "Proability of transition SI-R: " << current_P_SI_R << endl;
                cout << "Proability of transition SI-M: " << current_P_SI_M << endl;
            }
            ...
            if(sampled_state==0){ // Staying in S
                ...
                if(sampled_state==1){
                    ...
                    // Storing the I1 state in trajectories
                    mat_trajectories.matrix[ind*mat_trajectories.nb_columns+year+1]=1;
                    
                    if(DEBUG==0){
                        cout << "Year S-I " << year << " -- date " << vect_time_line->data[year] << endl;
                    }
                } else {
                    // Storing the M state in trajectories
                    for (unsigned int tmp_year=year; tmp_year<(n_years_follow_up); tmp_year++){
                        mat_trajectories.matrix[ind*mat_trajectories.nb_columns+tmp_year+1]=4;
                    }
                    
                    if(DEBUG==0){
                        cout << "Year S-M " << year << " -- date " << vect_time_line->data[year] << endl;
                    }
                }
            }
        ...
        if(year_SI!=0){
            ...
            unsigned int current_year=year_SI+1;
            
            // Get current sub-state of I (0 means S)
            unsigned int current_substate=0;
            
            while(current_year<(n_years_follow_up)){
                if(DEBUG==0){
                    cout << "\tYear " << current_year << " -- date " << vect_time_line->data[current_year] << endl;
                }
                ...
                mat_trajectories.matrix[ind*mat_trajectories.nb_columns+current_year+1]=current_substate+1;
...
//                if(current_substate < mat_P_Ii_Ij.nb_columns-2){ // Staying in one of the I sub-states
                current_year++;
                    ...
                    //                    mat_trajectories.matrix[ind*mat_trajectories.nb_columns+current_year+1]=current_substate;
//                }
//                else {
...
//
//                        // Storing the R state in trajectories
//                        //                        mat_trajectories.matrix[ind*mat_trajectories.nb_columns+current_year+1]=3;
//                    }
//                    //                    else {
//                    //                        // Otherwise: transition to M
//                    //                        mat_trajectories.matrix[ind*mat_trajectories.nb_columns+current_year+1]=4;
//                    //                    }
//                }
                vect_I_trans.clear();
            }
    ...
    for(unsigned int MyRow=0;MyRow<mat_trajectories.nb_rows;MyRow++){ // For loop over individuals
        for(unsigned int MyCol=0;MyCol<mat_trajectories.nb_columns;MyCol++){ // For loop over time points
            f_out_PSI << mat_trajectories.matrix[MyRow*mat_trajectories.nb_columns+MyCol]
            << " ";
        }
        f_out_PSI << endl;
    }
