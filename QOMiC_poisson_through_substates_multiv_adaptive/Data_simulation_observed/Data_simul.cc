...
int main(int argc, char *  argv[])
{
    ...
    for(unsigned int ind=0;ind<n_ind;ind++){
        ...
        while(Stop_simul==0 && year<(n_years_follow_up)){
            ...
            double current_P_SI_M=mat_P_SI_M.matrix[ind*mat_P_SI_M.nb_columns+year];
            ...
            vect_S_trans.push_back(current_P_SI_SI);
            vect_S_trans.push_back(current_P_SI_R);
            vect_S_trans.push_back(current_P_SI_M);
...
//                    mat_trajectories.matrix[ind*mat_trajectories.nb_columns+year+1]=3;
                    
                    for (unsigned int tmp_year=year; tmp_year<(n_years_follow_up); tmp_year++){
                        mat_trajectories.matrix[ind*mat_trajectories.nb_columns+tmp_year+1]=3;
                    }
                    
                    if(DEBUG==0){
                        cout << "Year SI-R " << year << " -- date " << vect_time_line->data[year] << endl;
                    }
                } else {
