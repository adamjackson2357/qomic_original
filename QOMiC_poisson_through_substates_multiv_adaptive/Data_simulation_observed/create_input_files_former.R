...
colnames(W)=init_year:(end_year+1)
...
dodiag_years=apply(W, 1, function(W_t){
  if ("R"%in%W_t){
    names(W_t)[min(which(W_t=="R"))-1]
  } else {
    1900-init_date
  }})
dodiags=as.numeric(dodiag_years)+init_date
dodiags_dates=as.numeric(paste0(dodiags, "0201")) # arbitrary month and day
...
dod_years=apply(W, 1, function(W_t){
  if ("M"%in%W_t){
    names(W_t)[min(which(W_t=="M"))-1]
  } else {
    1900-init_date
  }})
dod=as.numeric(dod_years)+init_date
dod_dates=as.numeric(paste0(dod, "0201")) # arbitrary month and day
